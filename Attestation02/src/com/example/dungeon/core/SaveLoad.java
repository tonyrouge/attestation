...
/*
    public static void saveAll( GameState s ) {
        try (FileOutputStream fileOut = new FileOutputStream( "saveAll.txt" );
             ObjectOutputStream out = new ObjectOutputStream(fileOut)) {
            out.writeObject( s ); // Сериализация объекта
            System.out.println("Объект успешно сериализован в myclass.ser");
        } catch (IOException i) {
            i.printStackTrace();
        }
...
/*
	public static GameState loadAll() {
        GameState s = new GameState();
		if (!Files.exists(SAVE)) {    // !!!!!!!!!!!!!!!!!!!!
            System.out.println("Сохранение не найдено.");
            return null;
        }
		try (FileInputStream fileIn = new FileInputStream( "saveAll.txt" );
             ObjectInputStream in = new ObjectInputStream( fileIn ) ) {

            s = ( GameState ) in.readObject(); // Чтение объекта из файла

        } catch (IOException i) {
            i.printStackTrace();
        } catch (ClassNotFoundException c) {
            System.out.println("Класс объекта не найден");
            c.printStackTrace();
        }
		return s;
    }
*/
    ...
    public static void load(GameState s) {
        ...
        try (BufferedReader r = Files.newBufferedReader(SAVE)) {
            ...
            System.out.println( "pp : " + Arrays.asList( pp ) );
            p.setName( pp[ 0 ] );
